...
unsigned int nStakeTargetSpacing = 20;			        // 15 sec block spacing for PoS
...
const string strMessageMagic = "Wonderful8Coin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Wonderful8Coin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Wonderful8Coin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, const CBlockIndex* pindex)
{
	...
	if(nPoWHeight > LAST_PURE_POW_BLOCK)
		return 0;
...
static const int64_t ifac1 = 10000;		
static const int64_t ifac2 = 909;	
...
int64_t GetProofOfWorkBonusRewardFactor(CBlockIndex* pindex)
{
	...
	int count = get8Counts(hash);

	// printf(">> count = %d, hash = %s\n", count, hash.ToString().c_str());
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, const CBlockIndex* pindex)
{
	...
	int64_t nSubsidy = nRewardCoinYear * nCoinAge / 365;
...
// Wonderful8Coin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Wonderful8Coin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Wonderful8Coin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Wonderful8Coin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Wonderful8Coin: track money supply and mint amount info
...
// Wonderful8Coin: total coin age spent in transaction, in the unit of coin-days.
...
// Wonderful8Coin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // Wonderful8Coin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Wonderful8Coin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Wonderful8Coin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // Wonderful8Coin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nPowHeight > LAST_PURE_POW_BLOCK)
        return DoS(100, error("AcceptBlock() : reject proof-of-work at height %d", nPowHeight));
    ...
    for (unsigned i = 1; i < vtx.size(); i++)
		if (vtx[i].IsCoinBase())
		{
			...
			{
				int64_t val1 = (prevBlock.vtx[0].vout[j].nValue * nBonusReward / 100 + 0.5 * COIN) / COIN;
				val1 *= COIN;
				if (val1 != vtx[i].vout[j].nValue ||
    ...
    // Wonderful8Coin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Wonderful8Coin: check proof-of-stake
    ...
    // Wonderful8Coin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Wonderful8Coin: getblocks may not obtain the ancestor block rejected
    ...
    // Wonderful8Coin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Wonderful8Coin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "There is a visual resemblance between two digits 88, and 囍, the double joy";
        CTransaction txNew;
        txNew.nTime = 1406066442;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1406066442;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3548664;
        if(fTestNet)
        {
            block.nNonce = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("5f9d25227b0c2d176166d529c4bc14dca8752d22570f6d3f099d0637e21eaa56"));
        block.print();
        ...
        // Wonderful8Coin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0xdc, 0xef, 0xfc, 0xfd };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Wonderful8Coin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Wonderful8Coin: tell downloading node about the latest block if it's
